import os, re, sys, asyncio, sqlite3, datetime, logging
...
try:
    ...
    if os.name == 'nt':
        import msvcrt; msvcrt.locking(lock_fp.fileno(), msvcrt.LK_NBLCK, 1)
    else:
        import fcntl; fcntl.flock(lock_fp, fcntl.LOCK_EX | fcntl.LOCK_NB)
except Exception: sys.exit(0)

API_ID = int(os.environ.get("TG_API_ID", "32206759"))
API_HASH = os.environ.get("TG_API_HASH", "7db3022378b608c86cad321de9eb3261")
DEFAULT_SESSION = ("1BVtsOH4Bu3Dx8amqNZg9wNASIzCrlUtAX1DaMmJdzk5vcFV9t_97e9xV7eUKIU56OUvHxM_aDfCQYbghrM7qZZBrUCzF3uT_"
...
def init_db():
    ...
    conn.cursor().execute("CREATE TABLE IF NOT EXISTS forwarded_deals (id INTEGER PRIMARY KEY AUTOINCREMENT, raw_url TEXT, resolved_url TEXT UNIQUE, canonical_id TEXT UNIQUE, title TEXT, price TEXT, discount_pct INTEGER, forwarded_at TIMESTAMP)")
...
def is_already_forwarded(raw_url, resolved_url=None, canonical_id=None):
    ...
    if c.fetchone(): conn.close(); return True
    if resolved_url:
...
def record_forwarded_deal(raw_url, resolved_url, canonical_id, title, price, discount_pct):
    try:
        ...
        conn.commit(); conn.close()
    except Exception as e: logger.warning("DB record warning: " + str(e))

def is_search_or_list_url(url):
    if not url: return True
    u = url.lower()
    if "hidden-keywords=" in u or "srs=" in u: return False
    if "/s?k=" in u or "/s?rh=" in u or "/b?" in u or "search?q=" in u or "collection-tab-name=" in u or "param=dwnk" in u: return True
    return False
...
def extract_primary_deal_url(text, buttons=None):
    if text:
        ...
        if m:
            ...
            if not is_search_or_list_url(clean_u): return clean_u
        for u in re.findall(r'(https?://[^\s]+)', text):
            clean_u = re.sub(r'[\)\],\.!]+$', '', u)
            if "readmore" not in clean_u.lower() and "read-more" not in clean_u.lower() and not is_search_or_list_url(clean_u): return clean_u
    if buttons:
        for row in buttons:
            for btn in row:
                if hasattr(btn, 'url') and btn.url:
                    clean_b = re.sub(r'[\)\],\.!]+$', '', btn.url)
                    if not is_search_or_list_url(clean_b): return clean_b
    return None
...
def clean_and_resolve_source_text(text):
    ...
    for line in text.split('\n'):
        line_str = line.strip()
        if not line_str or "read more" in line_str.lower() or "readmore" in line_str.lower(): continue
...
async def process_and_forward_deal(raw_url, event_msg):
    ...
    if is_search_or_list_url(raw_url) or is_already_forwarded(raw_url): return
    canonical_id = extract_canonical_id(raw_url)
    if canonical_id and is_already_forwarded(raw_url, canonical_id=canonical_id): return
    ...
    deal_info = {}
    try: deal_info = await check_deal_and_screenshot(raw_url, min_discount_pct=MIN_DISCOUNT_PERCENT)
    except Exception as e: logger.warning("Playwright warning: " + str(e))
    image_file_path = None; resolved_prod_url = resolve_url(raw_url)
    if deal_info.get("screenshot_path") and os.path.exists(deal_info["screenshot_path"]):
        image_file_path = deal_info["screenshot_path"]
        if deal_info.get("product_url"): resolved_prod_url = deal_info["product_url"]
    if not image_file_path and has_image:
        try: image_file_path = await client.download_media(event_msg.media, file=TEMP_IMG_DIR)
        except Exception: pass
    c_id = deal_info.get("canonical_id") or extract_canonical_id(resolved_prod_url) or canonical_id
    if is_already_forwarded(raw_url, resolved_url=resolved_prod_url, canonical_id=c_id): return
    ...
    except Exception as e: logger.error("Failed to send deal: " + str(e))
...
async def new_channel_post_handler(event):
    ...
    if primary_url: await process_and_forward_deal(primary_url, event.message)
