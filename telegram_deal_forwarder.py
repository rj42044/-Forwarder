...
MIN_DISCOUNT_PERCENT = 0        # Forward ALL deals regardless of discount percentage
...
def is_search_or_list_url(url):
    ...
    # Allow curated multi-ASIN coupon deal pages and brand storefronts (hidden-keywords or srs=)
    if "hidden-keywords=" in url_low or "srs=" in url_low:
        return False
        
    # Skip raw user search queries (/s?k= or search?q=)
...
async def process_and_forward_deal(raw_url, event_msg):
    ...
    if is_search_or_list_url(raw_url):
        logger.info("Skipping generic search list URL: " + str(raw_url))
        return

    if is_already_forwarded(raw_url):
        logger.info("Skipping already forwarded raw URL: " + str(raw_url))
        return

    canonical_id = extract_canonical_id(raw_url)
    if canonical_id and is_already_forwarded(raw_url, canonical_id=canonical_id):
        logger.info("Skipping already forwarded canonical ID: " + str(canonical_id))
        return
    ...
    if not image_file_path and has_image:
        try:
            ...
            logger.warning("Could not download attached Telegram image: " + str(e))
    ...
    if is_already_forwarded(raw_url, resolved_url=resolved_prod_url, canonical_id=c_id):
        logger.info("Skipping duplicate resolved product URL: " + str(c_id or resolved_prod_url))
        return
    ...
    try:
        logger.info("Sending real-time deal to @" + str(DESTINATION_BOT) + "...")
        ...
        logger.info("✅ Successfully forwarded deal to @" + str(DESTINATION_BOT) + "!")
    except Exception as e:
        logger.error("Failed to send deal to @" + str(DESTINATION_BOT) + ": " + str(e))
...
async def main():
    ...
    print("Real-time listener active for Deal Blast Shopping...")
